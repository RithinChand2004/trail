...
def click_pic():
    ...
    if img_file_buffer is not None:
        prediction = disease.classify(img_file_buffer)
        result1 = "Predicted skin disease is : " + prediction[0][0]
        return result1
...
def upload_file():
    ...
    if uploaded_file is not None:
        prediction = disease.classify(uploaded_file)
...
with tab1:
    ...
    with st.expander("What are Skin Diseases"):
        st.write("Skin diseases are conditions that affect your skin. These diseases may cause rashes, inflammation, itchiness or other skin changes. Some skin conditions may be genetic, while lifestyle factors may cause others. Skin disease treatment may include medications, creams or ointments, or lifestyle changes.")
    
    with st.expander("How does our bot work"):
        st.write("You need to either upload or click a picture of the suspected/infected area, our bot collects the picture, classifies it and then predicts the type of skin disease. Result will be given at the bottom along with the type of skin disease our bot also provides you some precautions and medications for the disease.")

    with st.expander("What Skin diseases can our bot find"):
        st.write("Skin Diseases that bot can identify:")
        st.write("1. Acne Back 2. Amelanotic Melanoma 3. Atypical Melanocytic naevius 4. Atypical Mycobacterial infection 5. Basal Cell Carcinoma 6. Dermatofiboroma 7. Epidermoid and Trichilemal Cyst 8. Facial acne")
    st.write("Either upload a picture or click a picture of the allergic area")
    
    
    
    
    option1 = st.selectbox(
     'Please select your preference of picture from below',
     ('Select','Browse your picture from device', 'Click a picture from camera'), key=1)
    
    if(option1 == 'Select'):
        st.write("Please select one from them")
    
    if (option1 == 'Browse your picture from device'):
        result = upload_file()
        st.write(result)
        
    if (option1 == 'Click a picture from camera'):
        result1=click_pic()
        st.write(result1)
...
with tab3:
    ...
    if st.button('Heart Disease Test Result'):
        ...
        if (heart_prediction[0] == 1):
          ...
          with st.expander("Symptoms (This column is showing up because your test results shows you are suffering from heart diseases.)"):
            st.write("rgrsf")
        else:
